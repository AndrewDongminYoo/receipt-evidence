...
5. **Verification (server, deterministic).**
   ...
   - Split item evidence is additionally bound by order: printing preserves row order, so items citing split evidence must agree on it between their name positions and their amount positions — compared as page, then line, then offset within the line — and one printed excerpt cannot back two items' halves, while two distinct excerpts on one OCR-merged line remain two claims.
