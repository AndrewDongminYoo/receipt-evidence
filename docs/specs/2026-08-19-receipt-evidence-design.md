...
5. **Verification (server, deterministic).**
   ...
   - Split item evidence is additionally bound by order: column flattening preserves row order, so items citing split evidence must agree on it between their name lines and their amount lines, and one printed line cannot back two items' halves.
