...
**Outstanding acceptance work:** Task 15's manual browser pass and Task 16 Step 4's camera and gallery pass have no recorded repository evidence. The latter writes to the operator's daily iPhone, so it needs explicit authorisation for each device operation. Task 17's hosted demo link is not implemented because no deployment target is configured.
